...
const Cartpage: React.FC = () => {
    ...
    const totalSelectedPrice = cart?.CartItems
                            ...
                            <tbody>
                                {cart?.CartItems && cart.CartItems.length > 0 ? (
                                    cart?.CartItems.map((item, index) => (
                                        <tr key={index} data-id={item.id} className="hover:bg-gray-100 hover:cursor-pointer transition" >
                                            ...
                                            <td className=''>
                                                <div className="flex items-center">
                                                    <div className="flex items-center justify-center w-[70px] h-[70px] shrink-0 m-2">
                                                        <img src={SampleImage} alt="" className="w-full h-full object-contain p-1 rounded" />
                                                    </div>
                                            ...
                                            <td className="text-center  text-persian-rose-600">₱ {item.total}</td>
                                            <td className="text-center  ">
                ...
                <div className='flex w-full bottom-0 border-t-2 px-4 py-5 items-center justify-center gap-2 bg-white'>
                    ...
                    <div className=' flex-grow flex items-center gap-2 justify-end'>
                        {selectedCarts.length > 0 && (
                            <p className='text-start text-sm '>Items: {selectedCarts.length}</p>
                        )}
                        <p className='text-start text-persian-rose-500 lg:text-base text-sm uppercase '>Total: </p>
                        <p className='text-persian-rose-500 text-xl font-bold text-end'>₱{totalSelectedPrice.toFixed(2)}</p>
                    </div>
